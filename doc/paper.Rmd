...
> **N.B.** A nice alternative for next time could be to have an hex grid, nice geometric advantages.   
...
```{r eval=FALSE}
  ...
  foreach(i = squares.intersecting.polys.ids,
            .packages = c("sf", "lwgeom"))  %dopar% {
              ...
              inters[["tot_area"]]<-sum(st_area(cut.Polygons.In.Squares.output[[1]]))
              inters[["per_cover"]]<-sum(st_area(cut.Polygons.In.Squares.output[[1]]))/st_area(squares[i, ])
              inters
            }
